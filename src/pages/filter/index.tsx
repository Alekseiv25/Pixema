...
import { loadMoviesByfilterAsyncAction } from "../../store/reducers/moviesReducer/actions";
...
const Filter = () => {
    ...
    const query = `sortField=${filter.sortBy}&sortType=-1&year=${filter.yearFilterFrom}-${filter.yearFilterTo}${filter.genre === '' ? '' : `${`&genres.name=${filter.genre}`}`}&rating.kp=${filter.ratingFilterFrom}-${filter.ratingFilterTo}`
